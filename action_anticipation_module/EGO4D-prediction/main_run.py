...
# %%
#
if 'llama' in text_generation_model.lower():
...
with open("{}/fho_lta_taxonomy.json".format(annotation_folder), "r") as f:
    tax = json.load(f)
vmap, nmap = annotation_utils.get_vnmap(tax)

# with open(annotation_folder + "/narration.json", "r") as f:
#     narrations = json.load(f)
narrations = None

pred_action = None # TODO
if args.action_feature == "egovlp":
    with open("../../action_recognition_module/recognition-model/forecasting-main/outputs_test_step.json", "r") as f:
...
# all_caption = annotation_utils.update_captions( TODO
#     all_caption, 
#     args.caption_file,
# )
...
if prompt_design in ["maxmargin", "semanticsim"]:
    ...
    if prompt_search_in_train:
        if use_narration_summary == False:
            with open("../../captioning_module/EILEV/samples/captions/caption_video_blip_intention_ncap4_train.json", "r") as f:
                new_caption = json.load(f)
        else:
            with open("../../captioning_module/EILEV/samples/captions/summary_caption_video_blip_intention_ncap4_train.json", "r") as f:
                new_caption = json.load(f)
    else:
        if use_narration_summary == False:
            with open("../../captioning_module/EILEV/samples/captions/caption_video_blip_intention_ncap4_val.json", "r") as f:
                new_caption = json.load(f)
        else:
            with open("../../captioning_module/EILEV/samples/captions/summary_caption_video_blip_intention_ncap4_val.json", "r") as f:
                new_caption = json.load(f)
    for key in new_caption:
        all_caption[key] = new_caption[key]
...
#         all_question[_question] = annotation_utils.update_captions(all_question[_question], all_action_path, non_exist_ok=True)
if use_narration_summary == False:
    with open(f"../../captioning_module/EILEV/samples/captions/caption_video_blip_intention_ncap4_{split}.json", "r") as f:
        all_question["intention"] = json.load(f)
else:
    with open(f"../../captioning_module/EILEV/samples/captions/summary_caption_video_blip_intention_ncap4_{split}.json", "r") as f:
        all_question["intention"] = json.load(f)
...
print(len(ann_train), len(ann_val), len(ann_torun))
...
if prompt_design == "GT":
    ...
    prompt = prompt_utils.get_prompt(
        ...
        use_narration_summary=use_narration_summary,
        device=device,
    )
...
# clips_torun = ['08a7fe30-3f6c-42f7-8b92-88df11e59175', # Cooking
# '3ecc5e01-ae26-46b5-80bf-a4a7ac29029d', # Painting
# '7eb6abcd-443e-4354-9742-6f5bf5bd9626', # Laundry
# 'f5b6d5f6-4fdb-464e-bc35-1f8d66999a37'] # Construction/renovation

torun = []
for clip_uid in ann_torun:
    # if clip_uid not in clips_torun:
    #     continue
...
def generate_per_clip_uid_action_idx(clip_uid_action_idx):
    ...
    print('Generating for clip_uid: ', clip_uid_action_idx)
    #try:
    tr_prompt = annotation_utils.get_tr_prompt(
        clip_uid=clip_uid, 
    ...
    if prompt_design in ["maxmargin", "semanticsim"]:
        captions_this, actions_this = tr_prompt.split("Narrations: ")[int(use_narration != "None")].split("Past actions: ") #TODO
        ...
        # captions_this, actions_this = tr_prompt.split("Objects present during the 8 past actions: ")[int(all_question["objects"] is not None)].split("Past actions: ")
        # prompt_full = prompt.format(captions=captions_this, actions=actions_this) if all_question["objects"] is not None else prompt.format(actions=actions_this)
        ...
        while cnt < ntot_example:
            ...
            if l >= 3*(20+nprev)  and i_foridx in range(i-nprev, i+nprev):
                continue
            temp_prompt = annotation_utils.get_tr_prompt(
                clip_uid=clip_uid, 
                i=i_foridx, 
                nprev=nprev, 
                annotations=ann_torun,
    ...
    while True:
        if args.use_narration != "None":
            ...
            #seperation_str = "Objects present during the 8 past actions: " # TODO
        ...
        for examples_this in zip(*[iter(examples)] * nexample):
            ...
            for x in gen:
                pred_this = []
                try:
                    #print("Generated text: " + x["generated_text"], flush=True)
                    ...
                    for vn in x["generated_text"][len(prompt_this):].split(".")[0].split("), "):
                        vfind, nfind = annotation_utils.get_parsed(vn, vmap, nmap)
