...
def get_tr_prompt(clip_uid, i, nprev, annotations, all_caption, clip2video, video2scenario,pred_action,tax,
    ...
    objects = all_question.get("objects", None)
    ...
    if use_narration == "GT":
        ...
        if use_narration_summary:
            na_prompt = all_caption[clip_uid + "_" + str(annotations[clip_uid][i]["action_idx"])]
        else:
    ...
    # if objects is not None: TODO
    #     tr_prompt += "\nObjects present during the 8 past actions: "
    #     set_detected_objects = [obj for lst in [objects[f'{clip_uid}_{action_idx}'][1] for action_idx in range(annotations[clip_uid][i]["action_idx"], (annotations[clip_uid][i]["action_idx"] + nprev))] for obj in lst]
    #     tr_prompt += ", ".join(list(set(set_detected_objects) - set(["hand"])))
    ...
    if is_past_action:
        if is_pred_action == "True":
            ...
            if objects is None:
                ...
                past_act = ["({}, {})".format(x["verb"].split("_(")[0].replace('_',' '), x["noun"].split("_(")[0].replace('_',' ')) for x in annotations[clip_uid][i:(i + nprev)]]
                past_obj = [objects[f'{clip_uid}_{action_idx}'][1] for action_idx in range(annotations[clip_uid][i]["action_idx"], (annotations[clip_uid][i]["action_idx"] + nprev))]
                for i in range(len(past_act)):
                    try:
                        past_obj[i].remove('hand')
                    except ValueError:
                        pass 
                tr_prompt += ", ".join([f"{past_act[i]} (Objects in the scene: {past_obj[i]})" for i in range(len(past_act))]) + ", "
    if not is_test:
