...
class FrozenInTime(BaseModel):
    def __init__(self,
        ...
        if video_params['model'] == "SpaceTimeTransformer":
            ...
            if arch_config == 'base_patch16_224':
                ...
                vit_model = torch.load("/cluster/scratch/azaera/models/vit_base_p16_224/jx_vit_base_p16_224-80ecf9dd.pth", map_location="cpu")
        ...
        if projection == 'minimal':
            if "Salesforce/blip2-opt-2.7b" in self.text_params['model']: 
                txt_proj = nn.Sequential(nn.ReLU(),
                                         nn.Linear(self.text_model.config.text_config.hidden_size, projection_dim),
                                         )
            else:
                txt_proj = nn.Sequential(nn.ReLU(),
        ...
        if load_checkpoint not in ["", None]:
            ...
            local_rank = 0 #int(os.environ['LOCAL_RANK']) #0
    ...
    def forward(self, data, video_only=False, text_only=False, return_embeds=True):
        if video_only:
    ...
    def compute_text(self, text_data):
        if self.text_params['model'].startswith('bert'):
            text_embeddings = self.text_model(text_data['input_ids'], attention_mask=text_data['attention_mask'])[
                'pooler_output']
        elif 'all-MiniLM' in self.text_params['model'] or 'all-mpnet-base-v2' in self.text_params['model']:
            model_output = self.text_model(**text_data)
            text_embeddings = self.mean_pooling(model_output, text_data['attention_mask'])
            #print(f'MiniLM output shape {text_embeddings.shape}') #(batch_size, 384)
        elif self.text_params['model'].startswith('distilbert'):
            #text_embeddings = self.text_model(**text_data).last_hidden_state[:, 0, :]
            model_output = self.text_model(**text_data)
            text_embeddings = self.mean_pooling(model_output, text_data['attention_mask'])
        elif 'roberta-base' in self.text_params['model']: 
            model_output = self.text_model(**text_data)
            text_embeddings = self.mean_pooling(model_output, text_data['attention_mask'])     
        elif 'clip-vit-base-patch32' in self.text_params['model']: 
            model_output = self.text_model(**text_data)
            text_embeddings = self.mean_pooling(model_output, text_data['attention_mask'])     
        elif "Salesforce/blip2-opt-2.7b" in self.text_params['model']: 
            model_output = self.text_model.get_text_features(**text_data, return_dict=True)
            text_embeddings = self.mean_pooling(model_output, text_data['attention_mask'])  
        else:
            raise NotImplementedError
