...
for noun_label in range(521):
    ...
    for clip_uid, action_idx in tqdm(to_run):
        ...
        try:
            logits = torch.load(os.path.join(ram_plus_base_outputs_path, 'aggregated_logits_hand_boxes_mapped_ego4d.pt'))  #trunc-mean-3_logits_hand_boxes_max-per-frame_mapped_ego4d.pt #max_logits_hand_boxes_mapped_ego4d.pt
            logits.requires_grad = False
        except:
            total += 1
            continue

        # Rescale logits using quantiles
        for i in range(len(logits)):
            quantiles = all_quantiles[i].tolist()[:5] # 0.1, 0.25, 0.5, 0.75, 0.9
            quantiles = [0] + quantiles + [1]
            fp = [0, 0.1, 0.25, 0.5, 0.75, 0.9, 1.0]
            logit = logits[i].item()
            logits[i] = np.interp(logit, quantiles, fp)

        if not os.path.exists(os.path.join(ram_plus_base_outputs_path, 'aggregated_logits_hand_boxes_mapped_ego4d_rescaled.pt')):
            print("SAVING")
            torch.save(logits, os.path.join(ram_plus_base_outputs_path, 'aggregated_logits_hand_boxes_mapped_ego4d_rescaled.pt'))
