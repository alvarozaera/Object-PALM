...
for noun_label in range(521):
    ...
    for clip_uid, action_idx in tqdm(to_run):
        ...
        try:
            logits = torch.load(os.path.join(ram_plus_base_outputs_path, 'aggregated_logits_hand_boxes_mapped_ego4d_rescaled.pt'))
        ...
        #ind = torch.where(logits >= global_thresh)[0]
        ind = torch.topk(logits, 15).indices
...
def plot_acc(corrects, totals, labels):
    ...
    for j in range(n_plots):
        ...
        ax.set_title(f'GT Noun Recall Using RAM++ with a Quantile-Based Transformation and Selecting the Top-15 Nouns (c) in Ego4D Validation Set ({j+1}/{n_plots})')
        ax.set_xticks(x)
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig(f'FINAL_REPORT_recall_per_noun_quantile_rescaled_top-15_{j}.png')
